...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Export the N2.177 MaterialSchema input and repair-flow audit as "
            "JSON or Markdown."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--flow",
        default="",
        help="Filter by material repair-flow id.",
    )
    args = parser.parse_args(argv)

    report = build_scene_material_repair_flow_audit_report(flow_id=args.flow)
    if args.format == "json":
...
def _report_markdown(report) -> str:
    ...
    lines = [
        "# MaterialSchema Input and Repair Flow Audit",
        "",
        f"- Status: {payload['status']}",
        f"- Repair flows: {counts['ready_flow_count']}/{counts['flow_count']}",
        f"- Capabilities: {counts['capability_count']}",
        f"- Material signals: {counts['material_signal_count']}",
        ...
        "",
        "| Flow | Status | Coverage | Packs | Families | Signals | Targets | Runtime | UI | Tests |",
        "| --- | --- | --- | ---: | ---: | --- | --- | --- | --- | --- |",
    ]
    for row in report.rows:
        lines.append(
            ...
            + " | ".join(
                [
                    _escape_markdown(row.flow_id),
                    _escape_markdown(row.status),
                    _escape_markdown(row.coverage_selector),
                    str(len(row.pack_ids)),
                    str(len(row.family_ids)),
                    _escape_markdown(_join_values(row.material_signal_ids)),
                    _escape_markdown(_join_values(row.repair_target_types)),
    ...
    for evidence in report.source_evidence:
        lines.append(
            "- "
            f"`{evidence.flow_id}/{evidence.evidence_id}`: "
            f"{evidence.status} ({evidence.evidence_layer}; {evidence.source_path})"
        )
    if report.issues:
        lines.extend(["", "## Issues", ""])
        for issue in report.issues:
            lines.append(f"- `{issue.flow_id}` / `{issue.kind}`: {issue.message}")
    return "\n".join(lines)
