...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Export the N2.180 report/artifact drilldown productization audit "
            "as JSON or Markdown."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--channel",
        default="",
        help="Filter by report/artifact drilldown channel id.",
    )
    args = parser.parse_args(argv)

    report = build_scene_report_artifact_drilldown_audit_report(
        drilldown_channel_id=args.channel,
        project_root=ROOT,
    )
    if args.format == "json":
...
def _report_markdown(report) -> str:
    ...
    lines = [
        "# Report/Artifact Drilldown Productization Audit",
        "",
        f"- Status: {payload['status']}",
        (
            "- Drilldown channels: "
            f"{counts['ready_drilldown_channel_count']}/"
            f"{counts['drilldown_channel_count']}"
        ),
        f"- Artifact kinds: {counts['artifact_kind_count']}",
        ...
        "",
        "| Channel | Status | Coverage | Packs | Families | Artifacts | Runtime | UI | Tests |",
        "| --- | --- | --- | ---: | ---: | --- | --- | --- | --- |",
    ]
    ...
    for evidence in report.source_evidence:
        lines.append(
            "- "
            f"`{evidence.drilldown_channel_id}/{evidence.evidence_id}`: "
    ...
    if report.issues:
        ...
        for issue in report.issues:
            lines.append(
                f"- `{issue.drilldown_channel_id}` / `{issue.kind}`: {issue.message}"
            )
    return "\n".join(lines)
