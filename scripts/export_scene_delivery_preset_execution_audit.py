...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Export the N2.176 DeliveryPreset execution completeness audit as "
            "JSON or Markdown."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--execution",
        default="",
        help="Filter by execution channel id.",
    )
...
def _report_markdown(report) -> str:
    ...
    lines = [
        "# DeliveryPreset Execution Completeness Audit",
        "",
        f"- Status: {payload['status']}",
        f"- Execution channels: {counts['ready_execution_channel_count']}/{counts['execution_channel_count']}",
        f"- Output signals: {counts['required_output_signal_count']}",
        f"- Payload keys: {counts['payload_key_count']}",
        ...
        "",
        "| Execution | Status | Coverage | Packs | Families | Signals | Payload | Runtime | Reports | UI | Tests |",
        "| --- | --- | --- | ---: | ---: | --- | --- | --- | --- | --- | --- |",
    ]
    for row in report.rows:
        lines.append(
            ...
            + " | ".join(
                [
                    _escape_markdown(row.execution_id),
                    _escape_markdown(row.status),
                    _escape_markdown(row.coverage_selector),
                    str(len(row.pack_ids)),
                    str(len(row.family_ids)),
                    _escape_markdown(_join_values(row.required_output_signal_ids)),
                    _escape_markdown(_join_values(row.payload_keys)),
                    _escape_markdown(_join_values(row.runtime_surface_ids)),
                    _escape_markdown(_join_values(row.report_surface_ids) or "-"),
    ...
    for evidence in report.source_evidence:
        lines.append(
            "- "
            f"`{evidence.execution_id}/{evidence.evidence_id}`: "
    ...
    if report.issues:
        ...
        for issue in report.issues:
            lines.append(
                f"- `{issue.execution_id}` / `{issue.kind}`: {issue.message}"
            )
    return "\n".join(lines)
