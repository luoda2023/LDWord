...
# Broad exception handling is intentional at this optional Office/COM boundary:
# a fallback failure must never corrupt or block the already-saved DOCX.
...
def _center_block_formula_paragraph(paragraph_el) -> None:
    paragraph_properties = paragraph_el.find(_w("pPr"))
    if paragraph_properties is None:
        paragraph_properties = etree.Element(_w("pPr"))
        paragraph_el.insert(0, paragraph_properties)
    justification = paragraph_properties.find(_w("jc"))
...
def apply_mathml_fallback_payloads_to_docx(doc_path: str, mathml_payloads: list[dict]) -> dict[str, int]:
    ...
    for paragraph in paragraphs:
        ...
        if not block:
            skipped_mixed += 1
            continue
        # A w:p can contain m:oMath but not m:oMathPara.  Centering the Word
        # paragraph preserves the block signal used by the equation-table pass.
        omml = convert_mathml_to_omml(mathml, block=False)
        if omml is None:
            continue
        if _replace_paragraph_with_omml(paragraph, omml):
            _center_block_formula_paragraph(paragraph)
            replaced += 1
...
def extract_mathtype_mathml_with_word(doc_path: str, timeout_sec: int = 30) -> tuple[bool, str, dict]:
    ...
    env["PYTHONUTF8"] = "1"
    cmd = build_office_broker_child_command(
...
def run_mathtype_office_child(doc_path: str) -> int:
    """Run the isolated Word/MathType extractor in a broker child process."""
    os.environ["DOCX_MATHTYPE_PATH"] = str(Path(doc_path).resolve())
    exec(  # noqa: S102 - controlled, source-owned child program
        compile(
            _PYWIN32_MATHTYPE_CHILD,
            "<mathtype-office-child>",
            "exec",
        ),
        {"__name__": "__mathtype_office_child__"},
    )
    return 0
...
def _main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--child-request", required=True)
    args = parser.parse_args(argv)
    return run_mathtype_office_child(args.child_request)
