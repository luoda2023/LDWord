...
OFFICIAL_MASTER_VERSION = "official-master-family-gbt-2026-07-11"
...
def write_official_master_family_docx(
    ...
    *,
    ...
    document.core_properties.subject = "Alavette Form official-document master"
...
def _configure_styles(document: Document) -> None:
    ...
    _set_style_east_asia(normal, "仿宋_GB2312")
...
def _add_upward_layout(document: Document) -> None:
    ...
    right.text = "签发人：{{@text:official_signer}}"
...
def _add_letter_layout(document: Document) -> None:
    _add_classification_reserve(document, compact=True)
    ...
    _add_formatted_run(
        ...
        east_asia="方正小标宋简体",
        ascii_font="FZXiaoBiaoSong-B05S",
        size=36,
        color="C00000",
    )
    _set_double_bottom_border(mark, color="C00000")

    doc_no = document.add_paragraph("{{@text:official_document_no}}")
    doc_no.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc_no.paragraph_format.space_before = Pt(6)
    doc_no.paragraph_format.space_after = Pt(20)

    _add_title_and_body(document, recipient=True, attachment=True, title_before=0)
    _add_closing(document)
    copy = document.add_paragraph("抄送：{{@text:official_copy_scope}}")
    copy.paragraph_format.space_before = Pt(14)
    copy.paragraph_format.first_line_indent = Cm(0)
    _set_paragraph_font(copy, east_asia="仿宋_GB2312", size=14)
    section = document.sections[0]
    for footer in (
...
def _add_minutes_layout(document: Document) -> None:
    ...
    _add_separator(document)
    _add_title(document, before=Pt(20), after=Pt(18))
...
def _add_order_layout(document: Document) -> None:
    ...
    doc_no = document.add_paragraph("{{@text:official_document_no}}")
    doc_no.alignment = WD_ALIGN_PARAGRAPH.CENTER
    doc_no.paragraph_format.space_after = Pt(24)
    _add_title(document, before=Pt(0), after=Pt(20))
    body = document.add_paragraph("{{@text:official_body}}")
    body.paragraph_format.first_line_indent = Cm(1.1)
    signer = document.add_paragraph("签发人：{{@text:official_signer}}")
...
def _add_classification_reserve(document: Document, *, compact: bool = False) -> None:
    ...
    for paragraph in (security, urgency):
        ...
        _set_paragraph_font(paragraph, east_asia="黑体", size=16, bold=True)
...
def _add_imprint(document: Document) -> None:
    ...
    table.autofit = False
    _set_table_grid(table, (Cm(3.4), Cm(12.2)))
    rows = (
        ("抄送：", "{{@text:official_copy_scope}}"),
        ("印发机关：", "{{@text:official_printing_org}}"),
        ("印发日期：", "{{@text:official_printing_date}}"),
    )
    for row, values in zip(table.rows, rows):
        for index, cell in enumerate(row.cells):
            ...
            _set_paragraph_font(paragraph, east_asia="仿宋_GB2312", size=14)
    _set_table_borders(
        table,
...
def _configure_page_number_footer(footer, *, odd: bool) -> None:
    ...
    _set_run_font(run, east_asia="宋体", ascii_font="SimSun", size=14)
...
def _set_double_bottom_border(paragraph, *, color: str) -> None:
    p_pr = paragraph._p.get_or_add_pPr()
    p_bdr = OxmlElement("w:pBdr")
    bottom = OxmlElement("w:bottom")
