...
from src.qt_api import QApplication, QByteArray, QColor, QIcon, QPainter, QPixmap, QSize, QSvgRenderer, Qt
...
# 协议: ISC License (兼容 MIT，允许商用/修改/分发)
...
SIDEBAR_ICONS = {
    ...
    "scene":       "mountain-snow",
    "template":    "scroll-text",
    "pipeline":    "sliders-horizontal",
    "assets":      "package",
    "theme":       "palette",
...
def _render_icon(name: str, size: int, color: str, dpr: float) -> QIcon:
    """内部缓存渲染器 — 相同参数不重复渲染 SVG。"""
...
def get_icon(
    ...
    return _render_icon(name, size, color, dpr)
