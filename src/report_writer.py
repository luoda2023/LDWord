...
def _format_application_section_word_limits_markdown(evidence: dict) -> list[str]:
    ...
    lines.append(f"- Matched limited sections: {int(summary.get('matched_section_count') or 0)}")
    lines.append(f"- Exceeded sections: {int(summary.get('exceeded_section_count') or 0)}")
    lines.append(f"- Required missing sections: {int(summary.get('required_missing_count') or 0)}")
    surfaces = _clean_list(evidence.get("word_surfaces", []))
    if surfaces:
        lines.append("- Word surfaces: " + _join_or_dash(surfaces))
