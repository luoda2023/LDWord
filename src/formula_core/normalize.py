...
_RE_PLAIN_WORD_ASSIGNMENT = re.compile(
    r"^(?P<left>[A-Za-z][A-Za-z0-9.-]{2,})\s*=\s*"
...
def looks_like_formula_text(text: str) -> tuple[bool, float, str]:
    ...
    # Configuration/status text is frequently written as ``status=ok`` or
    # ``timeout=30``. A bare word assignment has no reliable mathematical
    # evidence and must not be upgraded to OMML merely because it contains
    # ``=``. Compact variable equations (x=2, PV=nRT) and expressions with
    # scripts/operators continue through the formula path.
    if _RE_PLAIN_WORD_ASSIGNMENT.fullmatch(raw):
        return False, 0.0, "plain_text"
