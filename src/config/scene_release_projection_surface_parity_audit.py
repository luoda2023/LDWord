"""Release projection-surface parity audit for the scene matrix.

N2.388 checks that release-related scene-matrix audits are not only passing in
isolation, but are also projected through the user-visible and release-gate
surfaces that make them actionable. N2.391 backfills the projection ledger to
cover the later row-trace, subject-continuity, and closure-ledger surfaces.
"""
...
SCENE_RELEASE_PROJECTION_SURFACE_PARITY_REQUIRED_EVIDENCE_IDS: tuple[str, ...] = (
    "release_audit_source",
    "release_gate_check",
    "dashboard_source",
    "dashboard_card",
    "drilldown_item",
...
SCENE_RELEASE_PROJECTION_SURFACE_PARITY_SOURCE_MARKERS: tuple[
    tuple[str, str, tuple[str, ...]],
    ...
] = (
    (
        "release_projection_surface_parity_registry",
        "src/config/scene_release_projection_surface_parity_audit.py",
        (
            "SCENE_RELEASE_PROJECTION_SURFACE_PARITY_REQUIRED_EVIDENCE_IDS",
    ...
    (
        ...
        ("N2.394", "release_projection_surface_parity", "13/13"),
    ),
    (
...
@dataclass(frozen=True, slots=True)
class SceneReleaseProjectionSurfaceParityIssue:
    projection_id: str
    kind: str
    message: str
    severity: str = "error"
...
@dataclass(frozen=True, slots=True)
class SceneReleaseProjectionSurfaceParitySourceEvidence:
    source_id: str
    source_path: str
    markers: tuple[str, ...]
...
@dataclass(frozen=True, slots=True)
class SceneReleaseProjectionSurfaceParityRow:
    projection_id: str
    audit_source_id: str
    ...
    def is_ready(self) -> bool:
        return self.status == "projection_ready"
...
@dataclass(frozen=True, slots=True)
class SceneReleaseProjectionSurfaceParityAuditReport:
    rows: tuple[SceneReleaseProjectionSurfaceParityRow, ...]
    issues: tuple[SceneReleaseProjectionSurfaceParityIssue, ...]
    source_evidence: tuple[SceneReleaseProjectionSurfaceParitySourceEvidence, ...]
    ...
    @property
    def ready_projection_count(self) -> int:
    ...
    def release_gate_check_count(self) -> int:
        return sum(
            ...
            if "missing_release_gate_check" not in row.issue_ids
        )
    ...
    def dashboard_source_count(self) -> int:
        return sum(
            1
            for row in self.rows
            if "missing_dashboard_source" not in row.issue_ids
        )
    ...
    def summary_projection_count(self) -> int:
        return sum(
            1
            for row in self.rows
            if "missing_summary_projection" not in row.issue_ids
        )

    @property
    def export_script_count(self) -> int:
        return sum(1 for row in self.rows if "missing_export_script" not in row.issue_ids)

    @property
    def workflow_test_count(self) -> int:
        return sum(1 for row in self.rows if "missing_workflow_test" not in row.issue_ids)
    ...
    def to_payload(self) -> dict[str, object]:
        return {
            "status": self.status,
            "source_id": SCENE_RELEASE_PROJECTION_SURFACE_PARITY_AUDIT_SOURCE_ID,
            "required_evidence_ids": list(
                SCENE_RELEASE_PROJECTION_SURFACE_PARITY_REQUIRED_EVIDENCE_IDS
            ),
            "counts": {
                "projection_count": self.projection_count,
                "ready_projection_count": self.ready_projection_count,
...
def build_scene_release_projection_surface_parity_audit_report(
    *,
    project_root: Path | str | None = None,
) -> SceneReleaseProjectionSurfaceParityAuditReport:
    root = Path(project_root) if project_root is not None else Path.cwd()
    source_texts = _source_texts(root)
    rows = tuple(_row_for_spec(spec, root=root, source_texts=source_texts) for spec in SCENE_RELEASE_PROJECTION_SURFACE_SPECS)
    issues: list[SceneReleaseProjectionSurfaceParityIssue] = []
    for row in rows:
        issues.extend(
            SceneReleaseProjectionSurfaceParityIssue(
                row.projection_id,
                issue_id,
                f"Release projection surface {row.projection_id} failed: {issue_id}.",
            )
            for issue_id in row.issue_ids
        )
    source_evidence = _source_evidence(root)
    issues.extend(_source_evidence_issues(source_evidence))
    return SceneReleaseProjectionSurfaceParityAuditReport(
        rows=rows,
        issues=tuple(issues),
...
def audit_scene_release_projection_surface_parity_report(
    report: SceneReleaseProjectionSurfaceParityAuditReport | None = None,
) -> tuple[SceneReleaseProjectionSurfaceParityIssue, ...]:
    current = report or build_scene_release_projection_surface_parity_audit_report()
    return current.issues


def _row_for_spec(
    spec: SceneReleaseProjectionSurfaceSpec,
    *,
    root: Path,
    source_texts: dict[str, str],
) -> SceneReleaseProjectionSurfaceParityRow:
    issue_ids: list[str] = []
    if spec.audit_source_id not in source_texts["dashboard"]:
    ...
    return SceneReleaseProjectionSurfaceParityRow(
        projection_id=spec.projection_id,
        audit_source_id=spec.audit_source_id,
        ...
        status="projection_ready" if not issue_ids else "projection_gap",
        evidence_ids=(
            SCENE_RELEASE_PROJECTION_SURFACE_PARITY_REQUIRED_EVIDENCE_IDS
