...
class SceneMatrixDashboardReport:
    ...
    def _row_derived_count_payload(self) -> dict[str, int]:
        return {
    ...
    def _count_payload(self) -> dict[str, int]:
        counts: dict[str, int] = {}
        for field in fields(self):
            field_name = field.name
            if not field_name.endswith("_count") or field_name == "total_count":
                continue
            counts[field_name] = getattr(self, field_name)
            if field_name == "sample_fixture_count":
                counts.update(self._row_derived_count_payload())
        return counts
