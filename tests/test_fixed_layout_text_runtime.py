...
def test_entity_fill_replaces_content_control_and_textbox_placeholders():
    doc = Document()
    _append_content_control(doc, "{{company_name}}")
    _append_textbox(doc, "${legal_person}")
    ...
    assert "${legal_person}" not in text
...
def test_material_field_consistency_detects_fixed_layout_placeholder_residue():
    doc = Document()
    _append_content_control(doc, "甲方：{{party_a}}")
    ...
    assert item.placeholders_remaining == ("{{party_a}}",)
